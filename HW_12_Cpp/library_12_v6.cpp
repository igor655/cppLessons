...
char*** remoove_book_from_lib(char*** books, int& n)
{
	cout << " выбор книги для удаления remoove book from library\n";
	printBooks(books, n);
...
//???????????????????????????????????
void edit_book(char*** books, int& n)
{
	cout << "choose book to edit.\n";
	printBooks(books, n);

	int book_index{ 0 };
	cout << "Enter book number You want to edit: \n";
	cin >> book_index;
	cin.ignore(1, '\n');
	book_index--;
	...
	cin.getline(temp, 39);
	books[book_index][0] = new char[strlen(temp) + 1]; 
	strcpy_s(books[book_index][0], strlen(temp) + 1, temp);


	cout << "Введите автора книги : " << endl; // автор книги
	cin.getline(temp, 39);
	books[book_index][1] = new char[strlen(temp) + 1]; 
	strcpy_s(books[book_index][1], strlen(temp) + 1, temp);

	cout << " Введите  год книги: " << endl; //  год
	cin.getline(temp, 39);
	books[book_index][2] = new char[strlen(temp) + 1];
	strcpy_s(books[book_index][2], strlen(temp) + 1, temp);
...
void create_book(int& n, char*** books, int& fld)
{
...
void submenu(char*** books, int& n, int& fields)
{
	...
	books =	remoove_book_from_lib(books, n);
		//printBooks(books, n);  //неправильный вывод 
	}
	else if (chs == 3)
	{
		cout << " add book\n";
		create_book(n, books, fields);
...
int main()
{
	...
	char*** bookss = new char** [n];  // Создаем библиотеку с n книг
	char*** books = bookss;
...
Warning	C6386	Buffer overrun while writing to 'less_books':  the writable size is '(n-1)*4' bytes, but '8' bytes might be written.	FP_Lessons	C:\Users\bells\source\repos\FP_Lessons\FP_Lessons\hw_9.cpp	45
